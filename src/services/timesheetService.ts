...
const useMockData = false; //process.env.NEXT_PUBLIC_SKIP_MSAL === 'true' || 
  ...
  for (let weekOffset = 0; weekOffset < 4; weekOffset++) {
    ...
    for (let dayOffset = 0; dayOffset < 5; dayOffset++) {
      ...
      timeEntries.push({
        ...
        createdAt: format(entryDate, "yyyy-MM-dd'T'HH:mm:ss'Z'"),
        updatedAt: format(entryDate, "yyyy-MM-dd'T'HH:mm:ss'Z'")
      });
    ...
    const timesheet: Timesheet = {
      ...
      submissionDate: status !== 'draft' ? format(weekEndDate, 'yyyy-MM-dd') : undefined,
      approvalDate: status === 'approved' ? format(addDays(weekEndDate, 1), 'yyyy-MM-dd') : undefined,
      approvedByEmployeeId: status === 'approved' ? '1' : undefined,
      createdAt: format(weekStartDate, "yyyy-MM-dd'T'HH:mm:ss'Z'"),
      updatedAt: format(weekEndDate, "yyyy-MM-dd'T'HH:mm:ss'Z'"),
...
export const fetchMyTimesheets = async (employeeId?: string): Promise<Timesheet[]> => {
  if (useMockData) {
  ...
  const queryParams = employeeId ? `?employeeId=${encodeURIComponent(employeeId)}` : '';
  const response = await api.get(`/timesheets/my-timesheets${queryParams}`);
  return response.data;
};

export const 
fetchTimesheet = async (id: number): Promise<Timesheet> => {
  if (useMockData) {
    // Simulate network delay
...
export const createTimeEntry = async (timesheetId: number, data: CreateTimeEntryDto): Promise<TimeEntry> => {
  if (useMockData) {
    ...
    const newTimeEntry: TimeEntry = {
      ...
      project: mockProjects.find(p => p.id === data.projectId),
    ...
    timesheet.timeEntries.push(newTimeEntry);
